# College Bound Bears — site content

This repo holds **College Bound Bears's** website content only: `club.json`
and `images/`. Edit them via [Pages CMS](https://pagescms.org) (see
`.pages.yml`) or directly on GitHub.
...
CLUB_DATA_DIR=.. CLUB_SLUG=college-bound-bears node generator/build.js
...
which builds with `CLUB_SLUG=college-bound-bears`, `CLUB_DATA_DIR=..` and
publishes `generator/dist/` to GitHub Pages. The repo's Pages source
